...
@Directive({
    selector: '[xmaDisabledOnSelector]'
})
export class DisabledOnSelectorDirective implements OnChanges {
    @Input('xmaDisabledOnSelector') xmaDisabledOnSelector: string;
    ...
    ngOnChanges() {
        const elements = this.nativeElement.querySelectorAll(this.xmaDisabledOnSelector);
